...
static void	set_texture_img(t_data *data,
	t_texture_img *texture_img, char *path)
{
	texture_img->tex_img = mlx_xpm_file_to_image(data->init, path,
			&(texture_img->width), &(texture_img->length));
